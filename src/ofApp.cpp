...
void ofApp::setup(){
    ...
    camCapW = 1280*CAM_IMG_SCALE;
    camCapH = 960*CAM_IMG_SCALE;
    //camW = 1920*CAM_IMG_SCALE;
    //camH = 1080*CAM_IMG_SCALE;
    
    if(USE_LIVE_FEED) {
        // setup camera
        liveCam.setDeviceID(1);
        liveCam.initGrabber(camW, camH);
        //liveCam.initGrabber(camCapW, camCapH);
    }
    else {
        // load a snapshot
...
void ofApp::update(){
    ...
    if(USE_LIVE_FEED) {
        ...
        //colorImage.setFromPixels(liveCam.getPixels(), camCapW, camCapH);
        colorImage.rotate(180, colorImage.width/2, colorImage.height/2);
    } else {
        colorImage.setFromPixels(camSnapshot.getPixels(), camW, camH);
        //colorImage.setFromPixels(camSnapshot.getPixels(), camCapW, camCapH);
    }
    ...
    //contourFinder.setMaxArea(camCapW*camCapH);
...
void ofApp::draw(){
    ...
    if(USE_LIVE_FEED) {
        ...
        //artkGrayImage.draw(camCapW*CV_PREVIEW_SCALE, 0, drawW, drawH);
    } else {
...
void ofApp::saveFrameImage() {
    
    ofImage thisSnapshot;
    //ofImage thisAnalysisShot;
    
    thisSnapshot.setImageType(OF_IMAGE_COLOR);
    
    thisSnapshot.setFromPixels(liveCam.getPixels(), camW, camH, OF_IMAGE_COLOR);
    thisSnapshot.allocate(camW, camH,OF_IMAGE_COLOR);
    thisSnapshot.saveImage("Camera"+ofToString(frame)+".png",OF_IMAGE_QUALITY_BEST);
